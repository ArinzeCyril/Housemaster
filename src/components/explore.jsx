export default function explore({ Expl1, Expl2, Expl3, Expl4, Expl5, Expl6, Expl7, Expl8, Expl9, Expl10, Expl11, Expl12 }) {
  return (
    <div className="explore">
      <div className="expl-prptes">
        <h1>Explore our Properties</h1>
        <div className="expl-sd-logo">
          <p>
            Housemaster.
          </p>
        </div>
        <div className="expl-cards">
          <div className="expl-items  expl-text">
            <p>
              A Good Environment helps with Productivity
            </p>
            <hr className='expl-hr' />
          </div>
          <div className="expl-items grow-pix expl-ro2">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl1} alt="" />
              <p className='s-prptes'>
                7 Properties
              </p>
              <p className='h-prptes'>
                Vacation
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl2} alt="" />
              <p className='s-prptes'>
                7 Properties
              </p>
              <p className='h-prptes'>
                House
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl3} alt="" />
              <p className='s-prptes'>
                7 Properties
              </p>
              <p className='h-prptes'>
                House
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl4} alt="" />
              <p className='s-prptes'>
                7 Properties
              </p>
              <p className='h-prptes'>
                House
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl5} alt="" />
              <p className='s-prptes'>
                7 Properties
              </p>
              <p className='h-prptes'>
                House
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl6} alt="" />
              <p className='s-prptes'>
                7 Properties
              </p>
              <p className='h-prptes'>
                House
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl7} alt="" />
              <p className='s-prptes'>
                7 Properties
              </p>
              <p className='h-prptes'>
                House
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
        </div>
      </div>
      <hr className='expl-hr2' />
      <div className='expl-cty'>
        <h1>Explore Cities</h1>
        <div className="expl-cty-cards">
          <div className="expl-items grow-pix expl-col2">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img expl-col2-img' src={Expl8} alt="" />
              <p className='s-prptes'>
                Las Vegas
              </p>
              <p className='h-prptes'>
                7 Properties
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl9} alt="" />
              <p className='s-prptes'>
                Las Vegas
              </p>
              <p className='h-prptes'>
                7 Properties
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl10} alt="" />
              <p className='s-prptes'>
                Las Vegas
              </p>
              <p className='h-prptes'>
                7 Properties
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl11} alt="" />
              <p className='s-prptes'>
                Las Vegas
              </p>
              <p className='h-prptes'>
                7 Properties
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
          <div className="expl-items grow-pix">
            {/* <div className='expl-img-cntr'> */}
              <img className='expl-img' src={Expl12} alt="" />
              <p className='s-prptes'>
                Las Vegas
              </p>
              <p className='h-prptes'>
                7 Properties
              </p>
              <i className='i-prptes fa-regular fa-greater-than'></i>
            {/* </div> */}
          </div>
        </div>
      </div>
    </div>
  )
}
